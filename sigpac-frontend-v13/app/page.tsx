...
export default function Home() {
  ...
  const resetear = () => {
    ...
    setZonasData([]); setProduccion(null)
  }
  ...
  const handleMapClick = useCallback(async (lat: number, lon: number) => {
    ...
    try {
      ...
      setParcGeojson(data)
      setParcelaInfo(data.features?.[0]?.properties || {})
        ...
        {zonasData.length > 0 && (
          <>
            ...
            <section>
              ...
              {/* Leyenda zonas con superficie */}
              <div style={{ marginBottom: 12 }}>
                {(() => {
                  const totalHaZonas = zonasData.filter(z => z.pixeles > 0).reduce((acc, z) => acc + z.superficie_ha, 0)
                  return null
                })()}
                {zonasData.filter(z => z.pixeles > 0).map(z => {
                  const totalHaZonas = zonasData.filter(z => z.pixeles > 0).reduce((acc, z) => acc + z.superficie_ha, 0)
                  const zi = ZONAS_NDVI.find(zn => zn.zona === z.zona)!
                  const pct = totalHaZonas > 0 ? ((z.superficie_ha / totalHaZonas) * 100).toFixed(1) : '0'
                  return (
              ...
              {/* Inputs kg/ha */}
              <div style={{ fontSize: 10, color: 'var(--green)', fontFamily: 'var(--mono)', marginBottom: 8, letterSpacing: '0.06em', fontWeight: 700 }}>
                KG/HA ESPERADOS POR ZONA:
              </div>
              {zonasData.filter(z => z.pixeles > 0).map(z => {
                const zi = ZONAS_NDVI.find(zn => zn.zona === z.zona)!
                return (
                  <div key={z.zona} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    ...
                    <input
                      ...
                      onChange={e => setKgPorHa(prev => ({ ...prev, [String(z.zona)]: e.target.value }))}
                      style={{ flex: 1, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '5px 6px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}
                    />
                    <div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)', width: 30, textAlign: 'right' }}>kg</div>
                  </div>
                )
              })}

              <button onClick={calcularProduccion} disabled={calculandoProd} style={{ width: '100%', marginTop: 8, padding: '10px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoProd ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                {calculandoProd ? <><span className="spinner" style={{ borderTopColor: 'var(--bg)' }}/> CALCULANDO...</> : '🌾 CALCULAR PRODUCCIÓN'}
              </button>
            </section>
          </>
        )}
            ...
            <section>
              ...
              {produccion.zonas.filter((z: any) => z.kg_por_ha > 0).map((z: any) => (
                <div key={z.zona} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 10, fontFamily: 'var(--mono)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    ...
                    <span style={{ color: 'var(--muted)', fontSize: 9, opacity: 0.6 }}>{z.superficie_ha.toFixed(3)}ha · {z.kg_por_ha}kg/ha</span>
                  </div>
                  <span style={{ color: 'var(--text)', fontWeight: 700 }}>{z.kg_estimados.toLocaleString('es-ES')} kg</span>
                </div>
              ))}
              ...
              <div style={{ marginTop: 10, padding: '12px', borderRadius: 8, background: 'var(--green-dim)', border: '1px solid rgba(61,220,110,0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5, fontSize: 11, fontFamily: 'var(--mono)' }}>
                  ...
                  <span style={{ color: 'var(--text)' }}>{produccion.total_ha.toFixed(3)} ha</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5, fontSize: 11, fontFamily: 'var(--mono)' }}>
                  <span style={{ color: 'var(--muted)' }}>Total kilogramos</span>
                  <span style={{ color: 'var(--green)', fontWeight: 700 }}>{produccion.total_kg.toLocaleString('es-ES')} kg</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontFamily: 'var(--mono)', fontWeight: 700, marginTop: 8, paddingTop: 8, borderTop: '1px solid rgba(61,220,110,0.2)' }}>
                  ...
                  <span style={{ color: 'var(--green)', fontSize: 18 }}>{produccion.total_toneladas.toLocaleString('es-ES')} t</span>
                </div>
              </div>
            </section>
          </>
